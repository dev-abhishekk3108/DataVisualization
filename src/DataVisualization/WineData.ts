...
// Define a type for your dataItem
type DataItem = {
  [key: string]: number | string;
};

const wineData = data.map((dataItem: DataItem) => {
  const convertedDataItem: { [key: string]: number | string } = {};

  for (const key in dataItem) {
    const originalValue = dataItem[key];
    let newKey = key; // By default, keep the original key.

    // If the key is a string, remove spaces and use it as the new key.
    if (typeof key === 'string') {
      newKey = key.replace(/\s+/g, ''); // Remove spaces in the key name.
    }

    const value = parseFloat(originalValue as string);
    if (!isNaN(value)) {
      convertedDataItem[newKey] = value;
    } else {
      convertedDataItem[newKey] = originalValue;
    }
  }
